...
## [x] 配置したカードの効果が冗長かもしれない
- 現在、今配置したカードがわかるようにボード上、効果が設定してある
- これはいらないかもしれない
- というのは、おいたカードにはキャンセルアイコンがつくのでこれで十分かも
- 他にも選択したカードを示す効果もあるので、むしろ配置の効果を消す方がシンプルかもしれない
- **【完了】実装内容：**
  - 配置直後のカードの金色の枠線・グロー効果を削除（`CardComponent.css`）
  - 配置時のポップアニメーション（scale 1→1.15→1）を削除
  - キャンセルアイコン（↩️）は維持し、配置したカードの識別は可能
  - `isJustPlaced` propsを`CardComponent`, `Cell`から削除（不要になったため）
  - `BoardGrid`の`isJustPlaced()`関数は維持（キャンセルアイコン表示判定に使用）
  - 結果：UIがシンプルになり、キャンセルアイコンだけで配置カードを明示
  - 全109テスト成功
