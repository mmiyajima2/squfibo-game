...
## ゲーム終了条件と処理順（重要）
- ゲーム終了条件は以下のいずれか：
  1. **いずれかのプレイヤーが星を過半数（11個以上）獲得したとき**  
     → その時点で即座に決着（即時終了）
  2. 場に用意された星がすべて獲得されたとき
  3. 山札（デッキ）が0枚になったとき
...
- 途中でデッキや星が0枚／0個になっても、  
  その役は完全に解決される
- 星の獲得によって過半数（11個以上）に達した場合も、  
  その役の処理をすべて完了してからゲーム終了を判定する

---

## 勝敗判定
- **過半数（11個以上）の星を先に獲得したプレイヤーが即座に勝者となる**
- 過半数到達以外の終了条件（星がすべて獲得・山札が0枚）でゲームが終了した場合は、  
  各プレイヤーの星の数を比較し、星が最も多いプレイヤーが勝者
- 同点の場合は引き分け  
  （または追加ルールで決定してもよい）
